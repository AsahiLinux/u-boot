...
struct apple_dart_priv {
	...
	int ttbr_base;
	u32 ttbr_valid;
	void (*flush_tlb)(struct apple_dart_priv *);
};
...
static dma_addr_t apple_dart_map(struct udevice *dev, void *addr, size_t size)
{
	...
	for (i = 0; i < psize / DART_PAGE_SIZE; i++) {
		priv->l2[(dva / DART_PAGE_SIZE) + i] =
			(paddr  >> priv->shift) | DART_L2_VALID |
			DART_L2_START(0LL) | DART_L2_END(~0LL);
		paddr += DART_PAGE_SIZE;
	}
	flush_dcache_range((unsigned long)&priv->l2[dva / DART_PAGE_SIZE],
		   (unsigned long)&priv->l2[(dva / DART_PAGE_SIZE) + i]);
	priv->flush_tlb(priv);
...
static void apple_dart_unmap(struct udevice *dev, dma_addr_t addr, size_t size)
{
	...
	for (i = 0; i < psize / DART_PAGE_SIZE; i++) {
		priv->l2[(dva / DART_PAGE_SIZE) + i] = DART_L2_INVAL;
	}
	flush_dcache_range((unsigned long)&priv->l2[dva / DART_PAGE_SIZE],
		   (unsigned long)&priv->l2[(dva / DART_PAGE_SIZE) + i]);
	priv->flush_tlb(priv);
