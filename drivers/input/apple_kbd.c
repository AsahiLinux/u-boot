...
{
	struct apple_kbd_priv *priv = dev_get_priv(input->dev);
...
static void apple_kbd_service_key(struct input_config *input, int i,
				  int released)
{
	struct apple_kbd_priv *priv = dev_get_priv(input->dev);
	u8 *new;
			    ...
			    void *data, size_t size)
{
	...
	apple_kbd_service_modifiers(input);
	for (i = 0; i < sizeof(priv->new.keycode); i++) {
		apple_kbd_service_key(input, i, 1);
		apple_kbd_service_key(input, i, 0);
	}
