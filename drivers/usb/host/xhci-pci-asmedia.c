...
static int find_cpio_firmware(const char *path, void **fw, size_t *fw_size)
{
	struct cpio_hdr *c;
	ofnode node;
	u64 start, end;
	ulong namesize, filesize;
	const char *name;
	char *file;
	int ret;
	...
	ret = ofnode_read_u64(node, "linux,initrd-start", &start);
	if (ret)
		return -ENODEV;
	ret = ofnode_read_u64(node, "linux,initrd-end", &end);
	if (ret)
		return -ENODEV;

	c = (void *)start;
	while ((void *)c < (void *)end - sizeof(struct cpio_hdr)) {
		if (memcmp(c->c_magic, VMAGIC, sizeof(c->c_magic)) != 0)
			return -EINVAL;

		namesize = asc_ul(c->c_namesize);
		filesize = asc_ul(c->c_filesize);

		name = (char *)c + sizeof(struct cpio_hdr);
		file = (char *)c + roundup(sizeof(struct cpio_hdr) + namesize, 4);
		if (strlen(path) + 1 == namesize &&
		    strncmp(path, name, namesize) == 0) {
			*fw = file;
			*fw_size = filesize;
			return 0;
		}

		c = (struct cpio_hdr *)(file + roundup(filesize, 4));
	}
...
static void asmedia_write_reg(struct udevice *pdev, struct xhci_hccr *hccr, u16 addr, u8 data) {
...
{
	const char *fw_name = "vendorfw/asmedia/asm2214a-apple.bin";
	void *data;
	size_t len;
	int ret;
	...
	ret = find_cpio_firmware(fw_name, &data, &len);
	if (ret) {
	...
	ret = asmedia_load_fw(pdev, hccr, hcor, data, len);
	if (ret) {
