...
static int rtkit_handle_buf_req(struct apple_rtkit *rtk, int endpoint, struct apple_mbox_msg *msg)
{
	...
	case APPLE_RTKIT_EP_OSLOG:
		size = FIELD_GET(APPLE_RTKIT_OSLOG_SIZE, msg->msg0);
		break;
	default:
		size = FIELD_GET(APPLE_RTKIT_BUFFER_REQUEST_SIZE, msg->msg0) << 12;
		break;
	}
	...
	if (!buf->is_mapped) {
		...
		if (endpoint == APPLE_RTKIT_EP_OSLOG) {
			...
			msg->msg0 |= FIELD_PREP(APPLE_RTKIT_OSLOG_SIZE, size);
			msg->msg0 |= FIELD_PREP(APPLE_RTKIT_OSLOG_IOVA, buf->dva >> 12);
		} else {
			msg->msg0 = FIELD_PREP(APPLE_RTKIT_MGMT_TYPE,
					   APPLE_RTKIT_BUFFER_REQUEST);
			msg->msg0 |= FIELD_PREP(APPLE_RTKIT_BUFFER_REQUEST_SIZE, size >> 12);
